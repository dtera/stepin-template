<!--suppress JSUnusedGlobalSymbols -->
<script lang="ts">
  import axios from 'axios';
  import { ref } from 'vue';

  export const all7bColumns = [
    {
      title: '商品ID',
      dataIndex: 'id',
      key: 'id',
      width: '10%',
    },
    {
      title: '商品title',
      dataIndex: 'prodTitle',
      key: 'prodTitle',
      width: '20%',
    },
    {
      title: '商品图片',
      dataIndex: 'prodImgUrl',
      key: 'prodImgUrl',
      width: '10%',
    },
    {
      title: '品类',
      key: 'prodType',
      dataIndex: 'prodType',
      width: '6%',
    },
    // {
    //   title: '品牌',
    //   key: 'brandCnName',
    //   dataIndex: 'brandCnName',
    //   width: "6%",
    // },
    // {
    //   title: '商详',
    //   key: 'prodDetail',
    //   dataIndex: 'prodDetail',
    //   width: "20%",
    //   customCell: () => ({style: {wordBreak: 'break-all'}}),
    // },
    {
      title: '小店标准商品名',
      key: 'l2Name',
      dataIndex: 'l2Name',
      width: '9%',
    },
    /*{
title: '文章推理L3商品名',
key: 'bizL3Name',
dataIndex: 'bizL3Name',
width: "12%",
},
{
title: '文章推理品牌名',
key: 'bizBrand',
dataIndex: 'bizBrand',
width: "12%",
},*/
    {
      title: '商品最低价',
      key: 'skuMinPrice',
      dataIndex: 'skuMinPrice',
      width: '8%',
    },
    {
      title: '操作',
      key: 'action',
      width: '5%',
    },
  ];

  type APIParams = {
    pageNum?: number;
    pageSize?: number;
  };

  type APIResult = {
    records: {
      id: string;
      key: string;
      prodTitle: string;
      prodImgUrl: string;
      prodType: string;
      prodDetail: string;
      prodLlmPredNames: string;
    }[];
    total: number;
    size: number;
    current: number;
    pages: number;
  };

  export const all7bTotal = ref(0);

  export function apiAll7bList(method: string = 'list'): (params: APIParams) => Promise<object[]> {
    return async (params: APIParams): Promise<object[]> => {
      const res = await axios.get<APIResult>(`/api/all7b/${method}`, {
        params,
        headers: { 'Content-Type': 'application/json' },
      });

      // 假设 all7bTotal 是外部变量（如 Vue ref）
      all7bTotal.value = res.data.total;

      return res.data.records;
    };
  }
</script>
